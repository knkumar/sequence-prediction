    ...
    def training(self, filenames_train, fname):
        ...
        for _ in range(5000):
            ...
            if loss_val < min_loss:
                ...
                best_F = self.F
                best_G = self.G
                best_a_max = self.a_max
                best_evidence = self.evidence
                best_delay_var = [self.delay_var]
                best_weight_evidence = self.weight_evidence
                best_mu = self.mu
                best_sigma = self.sigma
                best_att_01 = self.attractor_dynamics_target[0,:,:]
                best_att_02 = self.attractor_dynamics_target[1,:,:]
                best_att_03 = self.attractor_dynamics_target[2,:,:]
                best_atf_01 = self.attractor_dynamics_foil[0,:,:]
                best_atf_02 = self.attractor_dynamics_foil[1,:,:]
                best_atf_03 = self.attractor_dynamics_foil[2,:,:]
                # os.exit()
        ...
        np.savetxt(f, self.sess.run(best_F), header="Parameter- F\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_G), header="Parameter- G\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_a_max), header="Parameter- a_max\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_evidence), header="Parameter- evidence\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_delay_var), header="Parameter- delay_var\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_weight_evidence), header="Parameter- weight_evidence\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_mu), header="Parameter- mu\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_sigma), header="Parameter- sigma\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_att_01), header="Parameter- attractor_dynamics 0_1\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_att_02), header="Parameter- attractor_dynamics 0_2\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_att_03), header="Parameter- attractor_dynamics 0_3\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_atf_01), header="Parameter- attractor_dynamics 1_1\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_atf_02), header="Parameter- attractor_dynamics 1_2\n", delimiter='\t')
        np.savetxt(f, self.sess.run(best_atf_03), header="Parameter- attractor_dynamics 1_3\n", delimiter='\t')
        f.close()
...
if __name__ == "__main__":
    ...
    # filenames_train = glob.glob('/home/kiran/projects/Kalman/data/tfrecords/S*train*block*')
